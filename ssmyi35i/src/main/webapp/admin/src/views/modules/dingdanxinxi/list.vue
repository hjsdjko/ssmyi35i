<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','订单领取') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="dingdanlingquCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'订单领取':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','订单领取') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="dingdanlingquCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'订单领取':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','订单领取') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dingdanlingquCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'订单领取':'' }}</el-button>
    ...
    <dingdanlingqu-cross-add-or-update v-if="dingdanlingquCrossAddOrUpdateFlag" :parent="this" ref="dingdanlingquCrossaddOrUpdate"></dingdanlingqu-cross-add-or-update>
...
import dingdanlingquCrossAddOrUpdate from "../dingdanlingqu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      dingdanlingquCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    dingdanlingquCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.dingdanlingquCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
      this.$refs.dingdanlingquCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.zhuangtaiOptions = "待领取,已领取".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
