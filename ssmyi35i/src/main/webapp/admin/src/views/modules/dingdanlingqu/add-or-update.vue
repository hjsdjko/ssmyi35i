<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否领取" prop="shifoulingqu">
          <el-select v-model="ruleForm.shifoulingqu" placeholder="请选择是否领取">
            <el-option
                v-for="(item,index) in shifoulingquOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否领取" prop="shifoulingqu">
	      <el-input v-model="ruleForm.shifoulingqu"
                placeholder="是否领取" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="领取时间" prop="lingqushijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.lingqushijian" label="领取时间" prop="lingqushijian">
              <el-input v-model="ruleForm.lingqushijian" 
                placeholder="领取时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dingdanlingqu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.dingdanlingquCrossAddOrUpdateFlag = false;
                  this.parent.search();
